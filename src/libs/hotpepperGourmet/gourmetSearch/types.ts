...
export const GourmetSearchShopSchema = z.object({
  ...
  ktai_coupon: z.number().describe('携帯用クーポン掲載 0:あり、1:なし'),
  ...
  capacity: z.number().optional().describe('総席数'),
  ...
  party_capacity: z.number().optional().describe('最大宴会収容人数'),
...
export const GourmetSearchResponseSchema = z.object({
  results: z.object({
    ...
    results_available: z.number().describe('検索結果の全件数'),
    results_returned: z.string().describe('検索結果の最大出力データ数'),
    results_start: z.number().describe('検索結果の開始位置'),
